...
### 5.1 Sistema de Dano & Combate Base (`NakeD.CombatCore`)
- [ ] **CharacterHealthManager.cs:** Converter `ApplyDamage`, `Start` para `virtual`. Criar `protected virtual void Die()`.
- [ ] **CharacterEquipmentManager.cs:** Converter `WeaponEquip`, `Attack`, `ToggleCombatMode`, `StartBlock`, `StopBlock`, `UnequipWeapon`, `ResetWeapon`, `Unequip` para `virtual`.
- [ ] **CharacterCombatAnimator.cs:** Converter `EquipWeapon`, `Attack`, `PerformAttack`, `ResetAttack`, `EnterCombatMode`, `ExitCombatMode`, `SetWeaponInHand`, `SetBlocking`, `Hit` para `virtual`.

### 5.2 Sistema de Armas de Fogo (`NakeD.RangedFramework`)
- [ ] **CharacterRangedCombat.cs:** Converter `HandleAimStarted`, `HandleAimCanceled`, `HandleAttackPressed`, `TryShoot`, `PerformShoot`, `EquipRangedWeapon`, `UnequipRangedWeapon`, `CanAimOrShoot` para `virtual`.
- [ ] **CharacterAimController.cs:** Converter `HandleAimStarted`, `HandleAimCanceled` para `virtual`.
- [ ] **RangedProjectile.cs:** Converter `Initialize`, `Update`, `OnCollisionEnter` para `virtual`.

### 5.3 Controle e Estado do Jogador (`NakeD.Template.TPS.Player`)
- [ ] **TargetingSystem.cs:** Converter `Update`, `FindNearestTarget`, `ClearTarget` para `virtual`.
- [ ] **PlayerAnimationUpdater.cs:** Converter `Update`, `UpdateAnimatorParameters` para `virtual`.
- [ ] **PlayerManager.cs:** Converter `Awake`, `Start` para `protected virtual`.
- [ ] **InputReader.cs:** Converter Callbacks (`OnMove`, `OnJump`, `OnAim`, etc) para `protected virtual`.

### 5.4 Sistema de Interação (`NakeD.InteractionSystem`)
- [ ] **InteractionScanner.cs:** Converter `Update`, `ScanForInteractables` para `virtual`.
